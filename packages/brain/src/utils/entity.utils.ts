...
export type UpdateEntity<T, ImmutableFields extends keyof T> = Partial<Omit<T, BaseEntityKeys | ImmutableFields>>;

export type CreateEntity<T, ImmutableFields extends keyof T, OptionalFields extends keyof T = never> = Omit<
  T,
  BaseEntityKeys | ImmutableFields | OptionalFields
> &
  Partial<Pick<T, OptionalFields>>;

//export type EntityCondition<E> = FindConditions<E> | FindConditions<E>[];

export const pickFieldName = <E, K extends keyof E = never>(_: Type<E>, ...keys: K[]): K[] => keys;
...
export function updateEntityType<T extends BaseEntity, K extends keyof T>(
  ...
  return PartialType(OmitType(classRef, [...immutableFields, ...baseEntityKeys], decorator), decorator);
}
